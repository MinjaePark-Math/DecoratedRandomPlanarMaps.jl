...
meta_base = Dict("model" => "uniform", "faces" => faces, "seed" => seed)
map_data = timed_step("generate_map") do
    generate_uniform_map(; faces=faces, seed=seed)
end
...
timed_step("export_svg") do
    export_svg_preview(
        ...
        pos2,
        joinpath(outroot, "uniform_2d.svg");
...
timed_step("export_web") do
    export_web_binaries(
        problem2.render_map_data,
        pos2,
        joinpath(outroot, "uniform_2d_web");
        edge_groups=problem2.edge_groups,
        faces=problem2.faces,
        triangles=problem2.surface_triangles,
        metadata=meta2,
...
timed_step("export_web_3d") do
    export_web_binaries(
        ...
        pos3 .* 10.0,
        joinpath(outroot, "uniform_3d_web");
...
timed_step("export_stl") do
    export_stl_binary(map_data, pos3 .* 100.0, joinpath(outroot, "uniform_3d.stl"))
end
